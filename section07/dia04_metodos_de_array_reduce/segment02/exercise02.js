...
// Crie uma função usando dados de estudantes, para mostrar na tela um relatório
// que diz em qual matéria a pessoa foi melhor. Você usará tanto o map quanto o
// reduce dentro dele.
...
const report = students.map(({ name, courses }) => {
  const course = courses.reduce((acc, course) =>
    acc.grade > course.grade ? acc : course
  );

  return { name, course: course.name };
});
